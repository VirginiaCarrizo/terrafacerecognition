        ...
        if faceCurFrame:
            for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
                ...
                if matches[matchIndex]:
                    ...
                    socketio.emit('dni_received', {'dni': dni})

                    # Abrir la página y completar el DNI en el input
                    url = "https://generalfoodargentina.movizen.com/pwa/inicio"
                    open_page_and_enter_dni(url, dni)

                    # Devolver el nombre y DNI del empleado
                    return jsonify({"nombre_apellido": employeeInfo['nombre_apellido'], "dni": dni})
...
def open_page_and_enter_dni(url, dni):
    """Abre la página en el navegador y completa el DNI en el campo de entrada."""
    # Abre la página en el navegador predeterminado
    webbrowser.open(url)
    print("Abriendo la página en el navegador...")
    time.sleep(5)  # Espera a que la página cargue

    # Obtener la posición del input y completar el DNI
    print("Posiciona el cursor sobre el input de DNI y espera 3 segundos.")
    time.sleep(3)
    # x, y = pyautogui.position()  # Captura la posición actual del cursor (debe estar sobre el input)
    x, y = (692, 710)  # Captura la posición actual del cursor (debe estar sobre el input)
    print(f"Posición del input obtenida: ({x}, {y})")
